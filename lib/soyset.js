...
SoySet.prototype.runScript = function (scriptData, path, cb) {
  try {
...
SoySet.prototype.getScriptData = function (scriptKey, done) {
  if (scriptKey === SoySet.SOY_UTILS_JS) {
    fs.readFile(this.options.soyUtilsJs, 'utf8', function (err, data) {
      ...
      // console.log(data);
      this.runScript(data, this.options.soyUtilsJs, done);
    }.bind(this));
  } else {
    this.runScript(this.soyJs.scriptSet[scriptKey], scriptKey, done);
  }
};
